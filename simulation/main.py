...
if __name__ == '__main__':
    ...
    ####################################
    # Calculate total iterations for the progress bar
    total_iterations = len(n_values) * len(t_values) * len(R_values) * len(Q_values) * len(m_values) * len(b_fractions) * len(k_values)
    ...
    with open(part3_csv_path, 'w', newline='') as file:
        ...
        writer.to_csv(file, index=False)
        for n, t, m, b_fraction, Q, k, l, a_fraction, R_all_value in tqdm(product(n_values, t_values, m_values, b_fractions, Q_values, k_values, l_values, a_fractions, R_all_range),total=total_iterations):
            ...
            part3_result = part3_simulator.simulate()
